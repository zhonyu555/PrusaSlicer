...
template <typename T>
static size_t get_struct_size(T &h)
{
    size_t tot_size = 0;
    boost::pfr::for_each_field(h, [&tot_size](auto &x) {
        tot_size += sizeof(x);
    });
    return tot_size;
}
                 ...
                 std::uint32_t              layer_count)
{
    ...
    slicer_info.lift_height2             = get_cfg_value<float_t>(cfg, "tsmc_lift_distance");
                               ...
                               const std::string    &/*projectname*/)
{
    ...
    header.small_preview_offset        = header.large_preview_offset + \
                                         get_struct_size(large_preview) + 
    ...
    try {
        ...
        size_t i = 0;
        unsigned long layer_data_offset = header.layer_table_offset + get_struct_size(layer_data) * \
                                          layer_count;
