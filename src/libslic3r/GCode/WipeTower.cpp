	...
	const int 				 new_temperature)
{
	...
	if (m_semm) {	
	// set toolchange temperature just prior to filament being extracted from melt zone and wait for set point (SKINNYDIP--normal mode only)
	if ((m_filpar[m_current_tool].filament_enable_toolchange_temp == true) && (m_filpar[m_current_tool].filament_use_fast_skinnydip == false)) {
    ...
    // for wrong extruder.
	// additionally, we are suppressing this temperature change if skinnydip fast temperature changing is active because it will happen later
	...
	if (m_semm) {
	    //Generate a skinnydip move after cooling moves complete
		...
		//ensure that proper hotend temperature is restored after skinnydip has finished meddling, honoring first layer temperature settings if applicable 
		
		if ((!m_is_first_layer) && (m_filpar[m_current_tool].filament_enable_toolchange_temp == true) && (m_filpar[m_current_tool].filament_use_fast_skinnydip == true)) {
			//begin to restore pre toolchange temp after skinnydip move completes without delay  (SKINNYDIP--fast method)
				writer.restore_pre_toolchange_temp(m_filpar[m_current_tool].temperature, true); //skinnydip restore hotend temperature, fast mode only
 		}
		
		//the following temperature change is suppressed if using skinnydip normal mode since it has already happened
		else if (m_is_first_layer && (m_filpar[m_current_tool].filament_enable_toolchange_temp == true) && (m_filpar[m_current_tool].filament_use_fast_skinnydip == true)){
				// obey first layer temperature setting by adding back first layer temp if applicable
				if (new_temperature != 0 && (new_temperature != m_old_temperature || m_is_first_layer) ) { 	
					writer.restore_pre_toolchange_temp(new_temperature, true); //skinnydip restore first layer hotend temperature, fast mode only
					//writer.set_extruder_temp(new_temperature, false);
					m_old_temperature = new_temperature;
				}			  
			}
