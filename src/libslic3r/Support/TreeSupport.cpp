...
MB_OK | MB_SYSTEMMODAL | MB_SETFOREGROUND | MB_ICONWARNING); #endif // TREE_SUPPORT_SHOW_ERRORS_WIN32
}
*/
                                                                    ...
                                                                    std::function<void()>      throw_on_cancel)
{
    ...
    tbb::parallel_for(tbb::blocked_range<LayerIndex>(1, num_overhang_layers), [&print_object, &config, &print_config, &enforcers_layers,
                                                                               &blockers_layers, support_auto, support_enforce_layers,
                                                                               support_threshold_auto, tan_threshold,
                                                                               enforcer_overhang_offset, num_raft_layers, &throw_on_cancel,
        ...
        for (LayerIndex layer_id = range.begin(); layer_id < range.end(); ++layer_id) {
            ...
            if (support_auto || enforced_layer) {
                ...
                overhangs = lower_layer_offset == 0 ? diff(current_layer.lslices, lower_layer.lslices) :
 ...
 * many points. \return A Polygons object containing the evenly spaced points. Does not represent an area, more a collection of points on lines.
 */
    ...
    for (Polyline part : input) {
        ...
        if (len < 2 * distance && min_points <= 1) {
            ...
            if (part.front() == part.back()) {
                ...
                // If the polyline was a polygon, there is a high chance it was an overhang. Overhangs that are <60� tend to be very thin
                // areas, so lets get the beginning and end of them and ensure that they are supported. The first point of the line will
                // always be supported, so rotate the order of points in this polyline that one of the two corresponding points that are
                // furthest from each other is in the beginning. The other will be manually added (optimal_end_index)
 ...
 */
[[nodiscard]] static Polygons safe_offset_inc(const Polygons &me,
                                              coord_t         distance,
                                              ...
                                              size_t          min_amount_offset)
{
    ...
    auto     collision_trimmed = [&collision_trimmed_buffer, &collision, &ret, distance]() -> const Polygons     &{
        if (collision_trimmed_buffer.empty() && !collision.empty())
            collision_trimmed_buffer = ClipperUtils::clip_clipper_polygons_with_subject_bbox(collision,
                                                                                                 get_extents(ret).inflated(
                                                                                                 std::max(0, distance) + SCALED_EPSILON));
...
int generate_raft_contact(const PrintObject &print_object, const TreeSupportSettings &config, InterfacePlacer &interface_placer)
{
    ...
    if (print_object.has_raft() && print_object.layer_count() > 0) {
        ...
        interface_placer.add_roof_unguarded(expansion > 0 ? expand(lslices, scaled<float>(expansion)) : to_polygons(lslices),
    ...
    RichInterfacePlacer &interface_placer)
{
    ...
    if (overhang_lines.empty()) {
        ...
        if (point_count <= min_support_points) {
            ...
            // angle of about < 60� so there is a fallback, as some support is better than none.
 ...
 */
static void generate_initial_areas(const PrintObject            &print_object,
                                   const TreeModelVolumes       &volumes,
                                   const TreeSupportSettings    &config,
                                   const std::vector<Polygons>  &overhangs,
                                   std::vector<SupportElements> &move_bounds,
                                   ...
                                   std::function<void()>         throw_on_cancel)
{
    ...
    // To ensure z_distance_top_layers are left empty between the overhang (zeroth empty layer), the support has to be added
    // z_distance_top_layers+1 layers below
    ...
    if (config.z_distance_top_layers > 0) {
        ...
        if (mesh_group_settings.support_angle > EPSILON && mesh_group_settings.support_angle < 0.5 * M_PI - EPSILON) {
            ...
            // half the external perimeter width). used by max_overhang_insert_lag, only if not min_xy_dist.
        ...
        for (size_t raw_overhang_idx = range.begin(); raw_overhang_idx < range.end(); ++raw_overhang_idx) {
            ...
            Polygons overhang_regular;
            {
                ...
                Polygons remaining_overhang =
                    intersection(diff(mesh_group_settings.support_offset == 0 ?
                                          overhang_raw :
                ...
                for (coord_t extra_total_offset_acc = 0;
                    ...
                    // supported at all.
                    remaining_overhang = diff(remaining_overhang, safe_offset_inc(overhang_regular, 1.5 * extra_total_offset_acc,
                                                                                  raw_collision, offset_step, 0, 1));
                    // Extending the overhangs by the inflated remaining overhangs.
                    overhang_regular = union_(overhang_regular, diff(safe_offset_inc(remaining_overhang, extra_total_offset_acc,
                                                                                     raw_collision, offset_step, 0, 1),
                                                                     relevant_forbidden));
 ...
 * it is ensured in-function! \param to_bp_data[out] The part of the Influence area that can reach the buildplate. \param to_model_data[out]
 * The part of the Influence area that do not have to reach the buildplate. This has overlap with new_layer_data. \param increased[out] Area
 * than can reach all further up support points. No assurance is made that the buildplate or the model can be reached in accordance to the
 * user-supplied settings. \param overspeed[in] How much should the already offset area be offset again. Usually this is 0. \param
 * mergelayer[in] Will the merge method be called on this layer. This information is required as some calculation can be avoided if they are
 * not required for merging. \return A valid support element for the next layer regarding the calculated influence areas. Empty if no
 * influence are can be created using the supplied influence area and settings.
 */
 ...
 * allowed to merge with a to model influence area. \param influence_areas[out] Area than can reach all further up support points. No
 * assurance is made that the buildplate or the model can be reached in accordance to the user-supplied settings. \param
 * bypass_merge_areas[out] Influence areas ready to be added to the layer below that do not need merging. \param last_layer[in] Influence
 * areas of the current layer. \param layer_idx[in] Number of the current layer. \param mergelayer[in] Will the merge method be called on
 * this layer. This information is required as some calculation can be avoided if they are not required for merging.
 */
            ...
            for (size_t merging_area_idx = range.begin(); merging_area_idx < range.end(); ++merging_area_idx) {
                ...
                for (const AreaIncreaseSettings &settings : order) {
                    ...
                    if (!settings.no_error) {
                        ...
                        // ensure that even if if wrongly would be a line, it still actually has an area that can be increased
...
#ifdef TREES_MERGE_RATHER_LATER
    intersect = intersect_small_with_bigger(merging_to_bp ? smaller_rad.areas.to_bp_areas : smaller_rad.areas.to_model_areas,
                                            merging_to_bp ? bigger_rad.areas.to_bp_areas : bigger_rad.areas.to_model_areas);
#endif
 ...
 * a circle of support may be placed. \param to_model_areas[in] The Elements of the current Layer that do not have to reach the buildplate.
 * Also contains main as every element that can reach the buildplate is not forced to. Value is the influence area where the center of a
 * circle of support may be placed. \param influence_areas[in] The influence areas without avoidance removed. \param insert_bp_areas[out]
 * Elements to be inserted into the main dictionary after the Helper terminates. \param insert_model_areas[out] Elements to be inserted into
 * the secondary dictionary after the Helper terminates. \param insert_influence[out] Elements to be inserted into the dictionary containing
 * the largest possibly valid influence area (ignoring if the area may not be there because of avoidance) \param erase[out] Elements that
 * should be deleted from the above dictionaries. \param layer_idx[in] The Index of the current Layer.
 */
 ...
 * that can reach the buildplate is not forced to. Value is the influence area where the center of a circle of support may be placed. \param
 * influence_areas[in] The Elements of the current Layer without avoidances removed. This is the largest possible influence area for this
 * layer. Value is the influence area where the center of a circle of support may be placed. \param layer_idx[in] The current layer.
 */
 ...
 */
static void create_layer_pathing(const TreeModelVolumes       &volumes,
                                 const TreeSupportSettings    &config,
                                 std::vector<SupportElements> &move_bounds,
                                 std::function<void()>         throw_on_cancel)
{
 ...
 * \param move_bounds[in,out] All currently existing influence areas
 */
static void create_nodes_from_area(const TreeModelVolumes       &volumes,
                                   const TreeSupportSettings    &config,
                                   std::vector<SupportElements> &move_bounds,
                                   std::function<void()>         throw_on_cancel)
{
 ...
 * branch area in layer_tree_polygons. \param inverse_tree_order[in] A mapping that returns the child of every influence area.
 */
...
// FIXME
            ...
            if (config.support_bottom_layers > 0 && !base_layer_polygons.empty()) {
                ...
                if (!floor_layer.empty()) {
                    if (support_bottom == nullptr)
                        support_bottom = &layer_allocate(layer_storage, SupporLayerType::BottomContact, print_object.slicing_parameters(),
                                                         config, layer_idx);
            ...
            if (!support_roof_polygons.empty()) {
                if (support_roof == nullptr)
                    support_roof = top_contacts[layer_idx] = &layer_allocate(layer_storage, SupporLayerType::TopContact,
                                                                             print_object.slicing_parameters(), config, layer_idx);
                support_roof->polygons = union_(support_roof_polygons);
            }
            if (!base_layer_polygons.empty()) {
                SupportGeneratorLayer *base_layer = intermediate_layers[layer_idx] = &layer_allocate(layer_storage, SupporLayerType::Base,
    ...
    for (std::pair<TreeSupportSettings, std::vector<size_t>> &processing : grouped_meshes) {
        ...
        if (has_support) {
            ...
            if (print_object.config().support_material_style == smsTree)
                draw_areas(*print.get_object(processing.second.front()), volumes, config, overhangs, move_bounds, bottom_contacts,
                           top_contacts, intermediate_layers, layer_storage, throw_on_cancel);
            else {
                assert(print_object.config().support_material_style == smsOrganic);
                organic_draw_branches(*print.get_object(processing.second.front()), volumes, config, move_bounds, bottom_contacts,
                                      top_contacts, interface_placer, intermediate_layers, layer_storage, throw_on_cancel);
            }
...
#endif // SLIC3R_DEBUG
            generate_support_layers(print_object, raft_layers, bottom_contacts, top_contacts, intermediate_layers, interface_layers,
