///|/ Copyright (c) Prusa Research 2020 - 2023 David Kocík @kocikdav, Vojtěch Bubník @bubnikv, Oleksandra Iushchenko @YuSanka, Filip Sykala
///@Jony01, Lukáš Matěna @lukasmatena, Enrico Turri @enricoturri1966, Lukáš Hejl @hejllukas, Tomáš Mészáros @tamasmeszaros
///|/
...
void NotificationManager::PopNotification::render(GLCanvas3D &canvas, float initial_y, bool move_from_overlay, float overlay_width)
{
    ...
    int flags = ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize |
...
void NotificationManager::PopNotification::count_lines()
{
    ...
    if (!m_hypertext.empty()) {
        int prev_end = m_endlines.size() > 1 ? m_endlines[m_endlines.size() - 2] : 0; // m_endlines.size() - 2 because we are fitting
    hypertext instead of last endline if (ImGui::CalcTextSize((escape_string_cstyle(text.substr(prev_end, last_end - prev_end)) +
    m_hypertext).c_str()).x > m_window_width - m_window_width_offset) { m_endlines.push_back(last_end); m_lines_count++;
        }
    }
    */
    ...
    ImGuiWrapper &imgui, const float text_x, const float text_y, const std::string text, bool more)
{
    ...
    // hover color
    ImVec4 green_color = ImVec4(0x1a / 255.0f, 0x74 / 255.0f, 0x76 / 255.0f, 1.0f);
    if (ImGui::IsItemHovered(ImGuiHoveredFlags_RectOnly))
        green_color.y += 0.2f;

    // text
    push_style_color(ImGuiCol_Text, green_color, m_state == EState::FadingOut, m_current_fade_opacity);
    ...
    ImGui::GetWindowDrawList()->AddLine(lineStart, lineEnd,
                                        IM_COL32((int) (green_color.x * 255), (int) (green_color.y * 255), (int) (green_color.z * 255),
                                                 (int) (green_color.w * 255.f *
...
void NotificationManager::ProgressBarWithCancelNotification::render_close_button_inner(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ImVec2 win_size(win_size_x, win_size_y);
    ImVec2 win_pos(win_pos_x, win_pos_y);
...
void NotificationManager::ProgressBarWithCancelNotification::render_cancel_button_inner(
    ImGuiWrapper &imgui, const float win_size_x, const float win_size_y, const float win_pos_x, const float win_pos_y)
{
    ImVec2 win_size(win_size_x, win_size_y);
    ImVec2 win_pos(win_pos_x, win_pos_y);
...
void NotificationManager::PrintHostUploadNotification::count_spaces()
{
    ...
    if (m_uj_state == UploadJobState::PB_ERROR || m_uj_state == UploadJobState::PB_COMPLETED_WITH_WARNING) {
        std::string text;
        // text = (m_data.level == NotificationLevel::ErrorNotificationLevel ? ImGui::ErrorMarker : ImGui::WarningMarker); its always
        // progressbar level (not error or warning)
...
void NotificationManager::UpdatedItemsInfoNotification::add_type(InfoItemType type)
{
    ...
    for (it = m_types_and_counts.begin(); it != m_types_and_counts.end(); ++it) {
        ...
        case InfoItemType::CustomSupports:
            text += format(_L_PLURAL("%1$d object was loaded with custom supports.", "%1$d objects were loaded with custom supports.",
                                     (*it).second),
                           (*it).second) +
                    "\n";
            break;
        case InfoItemType::CustomSeam:
            text += format(_L_PLURAL("%1$d object was loaded with custom seam.", "%1$d objects were loaded with custom seam.", (*it).second),
                           (*it).second) +
                    "\n";
            break;
        ...
        case InfoItemType::Sinking:
            text += format(_L_PLURAL("%1$d object was loaded with partial sinking.", "%1$d objects were loaded with partial sinking.",
                                     (*it).second),
                           (*it).second) +
                    "\n";
            break;
                                                    ...
                                                    std::function<bool(wxEvtHandler *)> callback)
{
    ...
    for (std::unique_ptr<PopNotification> &notification : m_pop_notifications) {
        ...
        if (type == NotificationType::NoNewReleaseAvailable && (notification->get_type() == NotificationType::NewAlphaAvailable ||
    ...
    int id, float filesize, const std::string &filename, const std::string &host, float percentage)
{
    ...
    // filename is created from boost::filesystem::path.string() which if created by path / "file" return \\ as folder division. But could
    // also contain / as folder division. Lets unite this into "/" only.
...
void NotificationManager::push_download_progress_notification(const std::string &text, std::function<bool()> cancel_callback)
{
    ...
    push_notification_data(std::make_unique<NotificationManager::ProgressBarWithCancelNotification>(data, m_id_provider, m_evt_handler,
                                                                                                    cancel_callback),
                           0);
}
