...
bool Bundle::load(fs::path source_path,
                  bool     ais_in_resources,
                  bool     ais_prusa_bundle)
{
    ...
    // Throw when parsing invalid configuration. Only valid configuration is
    // supposed to be provided over the air.
    auto [config_substitutions, presets_loaded] =
        preset_bundle->load_configbundle(
            path_string, PresetBundle::LoadConfigBundleAttribute::LoadSystem,
            ForwardCompatibilitySubstitutionRule::Disable);
    UNUSED(config_substitutions);
    // No substitutions shall be reported when loading a system config bundle,
    ...
    if (first_vendor == preset_bundle->vendors.end()) {
        BOOST_LOG_TRIVIAL(error)
            << boost::format("Vendor bundle: `%1%`: No vendor information "
                             "defined, cannot install.") %
                   path_string;
        return false;
    }
    if (presets_loaded == 0) {
        BOOST_LOG_TRIVIAL(error)
            << boost::format("Vendor bundle: `%1%`: No profile loaded.") %
                   path_string;
        return false;
    }

    BOOST_LOG_TRIVIAL(trace)
        << boost::format("Vendor bundle: `%1%`: %2% profiles loaded.") %
               path_string % presets_loaded;
...
BundleMap BundleMap::load()
{
    ...
    const auto vendor_dir = (boost::filesystem::path(Slic3r::data_dir()) /
                             "vendor")
                                .make_preferred();
    const auto rsrc_vendor_dir = (boost::filesystem::path(resources_dir()) /
                                  "profiles")
                                     .make_preferred();

    auto prusa_bundle_path = (vendor_dir / PresetBundle::PRUSA_BUNDLE)
                                 .replace_extension(".ini");
    auto prusa_bundle_rsrc = false;
    if (!boost::filesystem::exists(prusa_bundle_path)) {
        prusa_bundle_path = (rsrc_vendor_dir / PresetBundle::PRUSA_BUNDLE)
                                .replace_extension(".ini");
        prusa_bundle_rsrc = true;
    }
    {
        Bundle prusa_bundle;
        if (prusa_bundle.load(std::move(prusa_bundle_path), prusa_bundle_rsrc,
                              true))
    ...
    for (auto dir : {&vendor_dir, &rsrc_vendor_dir}) {
        for (const auto &dir_entry :
            ...
            if (Slic3r::is_ini_file(dir_entry)) {
                std::string id = dir_entry.path()
                                     .stem()
                                     .string(); // stem() = filename() without
                                                // the trailing ".ini" part
...
Bundle &BundleMap::prusa_bundle()
{
    ...
    if (it == end()) {
        throw Slic3r::RuntimeError("ConfigWizard: Internal error in "
                                   "BundleMap: PRUSA_BUNDLE not loaded");
    }
...
struct PrinterPickerEvent : public wxEvent
{
    ...
    PrinterPickerEvent(wxEventType eventType,
                       int         winid,
                       std::string vendor_id,
                       std::string model_id,
                       std::string variant_name,
...
const std::string PrinterPicker::PRINTER_PLACEHOLDER =
    ...
    : wxPanel(parent), vendor_id(vendor.id), width(0)
{
    ...
    // wxGrid appends widgets by rows, but we need to construct them in
    // columns. These vectors are used to hold the elements so that they can
    // be appended in the right order.
    ...
    for (const auto &model : models) {
        ...
        wxBitmap bitmap;
        int      bitmap_width = 0;
        auto load_bitmap = [](const wxString &bitmap_file, wxBitmap &bitmap,
                              int &bitmap_width) -> bool {
        ...
        if (!load_bitmap(GUI::from_u8(Slic3r::data_dir() + "/vendor/" +
                                      vendor.id + "/" + model.id +
                                      "_thumbnail.png"),
                         bitmap, bitmap_width)) {
            if (!load_bitmap(GUI::from_u8(Slic3r::resources_dir() +
                                          "/profiles/" + vendor.id + "/" +
                                          model.id + "_thumbnail.png"),
                             bitmap, bitmap_width)) {
                BOOST_LOG_TRIVIAL(warning)
                    << boost::format(
                           "Can't find bitmap file `%1%` for vendor `%2%`, "
                           "printer `%3%`, using placeholder icon instead") %
                           (Slic3r::resources_dir() + "/profiles/" +
                            vendor.id + "/" + model.id + "_thumbnail.png") %
                           vendor.id % model.id;
                load_bitmap(Slic3r::var(PRINTER_PLACEHOLDER), bitmap,
                            bitmap_width);
            }
        }
        auto *title = new wxStaticText(this, wxID_ANY, from_u8(model.name),
        ...
        for (size_t i = 0; i < model.variants.size(); i++) {
            ...
            const auto label = model.technology == ptFFF ?
                                   from_u8((boost::format("%1% %2% %3%") %
                                            variant.name % _utf8(L("mm")) %
                                            _utf8(L("nozzle")))
                                               .str()) :
                                   from_u8(model.name);

            if (i == 1) {
                auto *alt_label = new wxStaticText(variants_panel, wxID_ANY,
                                                   _L("Alternate nozzles:"));
                alt_label->SetFont(font_alt_nozzle);
                variants_sizer->Add(alt_label, 0, wxBOTTOM, 3);
                is_variants = true;
            }

            auto *cbox = new Checkbox(variants_panel, label, model_id,
                                      variant.name);
            i == 0 ? cboxes.push_back(cbox) : cboxes_alt.push_back(cbox);

            const bool enabled = appconfig.get_variant(vendor.id, model_id,
                                                       variant.name);
    ...
    if (titles.size() > 1 || is_variants) {
        // It only makes sense to add the All / None buttons if there's
        // multiple printers All Standard button is added when there are more
        // variants for at least one printer
        auto *sel_all_std = new wxButton(this, wxID_ANY,
                                         titles.size() > 1 ?
                                             _L("All standard") :
                                             _L("Standard"));
        ...
        if (is_variants)
            sel_all_std->Bind(wxEVT_BUTTON,
                              [this](const wxCommandEvent &event) {
                                  this->select_all(true, false);
                              });
        sel_all->Bind(wxEVT_BUTTON, [this](const wxCommandEvent &event) {
            this->select_all(true, true);
        });
        sel_none->Bind(wxEVT_BUTTON, [this](const wxCommandEvent &event) {
        ...
        if (is_variants)
            m_button_indexes = {sel_all_std->GetId(), sel_all->GetId(),
                                sel_none->GetId()};
        else {
            sel_all_std->Destroy();
...
std::set<std::string> PrinterPicker::get_selected_models() const
{
    ...
    for (const auto &cb : cboxes)
        if (cb->GetValue()) ret_set.emplace(cb->model);
...
{
    PrinterPickerEvent evt(EVT_PRINTER_PICK, GetId(), vendor_id, cbox->model,
                           cbox->variant, checked);
    AddPendingEvent(evt);
}

// Wizard page base

ConfigWizardPage::ConfigWizardPage(ConfigWizard *parent,
                                   wxString      title,
                                   wxString      shortname,
                                   unsigned      indent)
    ...
    , indent(indent)
{
    ...
    auto      *text = new wxStaticText(this, wxID_ANY, std::move(title),
                                       wxDefaultPosition, wxDefaultSize,
                                       wxALIGN_LEFT);
    ...
    // see https://github.com/prusa3d/PrusaSlicer/issues/5103 and
    // https://github.com/prusa3d/PrusaSlicer/issues/4861 So, non-active pages
    // will be hidden later, on wxEVT_SHOW, after completed Layout() for all pages
    if (!wxLinux_gtk3) this->Hide();
...
{
    auto *widget = new wxStaticText(this, wxID_ANY, text, wxDefaultPosition,
    ...
    : ConfigWizardPage(
          ...
          from_u8((boost::format(
                   ...
                   SLIC3R_APP_NAME)
                      .str()),
          _L("Welcome"))
    , welcome_text(append_text(
          from_u8((boost::format(
                       _utf8(L("Hello, welcome to %s! This %s helps you with "
                               "the initial configuration; just a few "
                               "settings and you will be ready to print."))) %
                   SLIC3R_APP_NAME % _utf8(ConfigWizard::name()))
                      .str())))
    , cbox_reset(append(new wxCheckBox(
          this,
          wxID_ANY,
          _L("Remove user profiles (a snapshot will be taken beforehand)"))))
    , cbox_integrate(append(
          new wxCheckBox(this,
                         wxID_ANY,
                         _L("Perform desktop integration (Sets this binary "
                            "to be searchable by the system)."))))
{
    welcome_text->Hide();
    cbox_reset->Hide();
    cbox_integrate->Hide();
    ...
    , install(false) // only used for 3rd party vendors
{
    ...
    for (const auto &family : families) {
        ...
        if (std::find_if(vendor.models.begin(), vendor.models.end(),
                         filter) == vendor.models.end()) {
            continue;
        }

        const auto picker_title =
            family.empty() ?
                wxString() :
                from_u8((boost::format(_utf8(L("%s Family"))) % family).str());
        auto *picker = new PrinterPicker(this, vendor, picker_title, MAX_COLS,
                                         *appconfig, filter);

        picker->Bind(EVT_PRINTER_PICK,
                     [this, appconfig](const PrinterPickerEvent &evt) {
                         appconfig->set_variant(evt.vendor_id, evt.model_id,
                                                evt.variant_name, evt.enable);
...
int PagePrinters::get_width() const
{
    return std::accumulate(printer_pickers.begin(), printer_pickers.end(), 0,
                           [](int acc, const PrinterPicker *picker) {
                               return std::max(acc, picker->get_width());
                           });
}
...
void PagePrinters::set_run_reason(ConfigWizard::RunReason run_reason)
{
    if (is_primary_printer_page &&
        (run_reason == ConfigWizard::RR_DATA_EMPTY ||
         run_reason == ConfigWizard::RR_DATA_LEGACY) &&
...
PageMaterials::PageMaterials(ConfigWizard *parent,
                             Materials    *materials,
                             wxString      title,
                             wxString      shortname,
                             wxString      list1name)
    ...
    , list_profile(new PresetList(this))
{
    ...
    list_profile->Bind(wxEVT_CHECKLISTBOX, [this](wxCommandEvent &evt) {
    ...
    sel_all->Bind(wxEVT_BUTTON,
                  [this](wxCommandEvent &) { select_all(true); });
    sel_none->Bind(wxEVT_BUTTON,
                   [this](wxCommandEvent &) { select_all(false); });
    /*
    Bind(wxEVT_PAINT, [this](wxPaintEvent& evt) {on_paint();});

    list_profile->Bind(wxEVT_MOTION, [this](wxMouseEvent& evt) {
    on_mouse_move_on_profiles(evt); }); list_profile->Bind(wxEVT_ENTER_WINDOW,
    [this](wxMouseEvent& evt) { on_mouse_enter_profiles(evt); });
    list_profile->Bind(wxEVT_LEAVE_WINDOW, [this](wxMouseEvent& evt) {
    on_mouse_leave_profiles(evt); });
    */
    reload_presets();
...
void PageMaterials::set_compatible_printers_html_window(
    const std::vector<std::string> &printer_names, bool all_printers)
{
    const auto bgr_clr =
...
#endif
#endif
    const auto bgr_clr_str  = wxString::Format(wxT("#%02X%02X%02X"),
                                               bgr_clr.Red(), bgr_clr.Green(),
                                               bgr_clr.Blue());
    const auto text_clr     = wxGetApp().get_label_clr_default();
    const auto text_clr_str = wxString::Format(wxT("#%02X%02X%02X"),
                                               text_clr.Red(),
                                               text_clr.Green(),
                                               text_clr.Blue());
    wxString   first_line =
        format_wxstr(_L("%1% marked with <b>*</b> are <b>not</b> compatible "
                        "with some installed printers."),
                     materials->technology == T_FFF ? _L("Filaments") :
                                                      _L("SLA materials"));
    wxString text;
    if (all_printers) {
        wxString second_line =
            format_wxstr(_L("All installed printers are compatible with the "
                            "selected %1%."),
                         materials->technology == T_FFF ? _L("filament") :
                                                          _L("SLA material"));
        text = wxString::Format("<html>"
                                ...
                                "</body>"
                                "</html>",
                                bgr_clr_str, text_clr_str, first_line,
                                second_line);
    } else {
        wxString second_line;
        if (!printer_names.empty())
            second_line =
                (materials->technology == T_FFF ?
                     _L("Only the following installed printers are "
                        "compatible with the selected filaments") :
                     _L("Only the following installed printers are "
                        "compatible with the selected SLA materials")) +
                ":";
        text = wxString::Format("<html>"
                                "<style>"
                                "table{border-spacing: 1px;}"
        ...
        for (size_t i = 0; i < printer_names.size(); ++i) {
            text += wxString::Format("<td>%s</td>",
                                     boost::nowide::widen(printer_names[i]));
            if (i % 3 == 2) {
...
{
    if (sel_material == last_hovered_item) return;
    if (sel_material == -1) {
    ...
    // get material preset
    const std::vector<const Preset *> matching_materials =
        materials->get_presets_by_alias(material_name);
    ...
    for (const Preset *printer : materials->printers) {
        for (const Preset *material : matching_materials) {
            if (is_compatible_with_printer(
                    PresetWithVendorProfile(*material, material->vendor),
    ...
    set_compatible_printers_html_window(names, names.size() ==
                                                   materials->printers.size());
}

void PageMaterials::update_lists(int sel_type,
                                 int sel_vendor,
                                 int last_selected_printer /* = -1*/)
{
...
#else
    auto are_equal = [](const wxArrayInt &arr_first,
                        const wxArrayInt &arr_second) {
        if (arr_first.GetCount() != arr_second.GetCount()) return false;
        for (size_t i = 0; i < arr_first.GetCount(); i++)
            if (arr_first[i] != arr_second[i]) return false;
        return true;
    };
        ...
        if (sel_printers_count > 0) {
            ...
            if (sel_printers[0] == 0 && sel_printers_count > 1) {
                if (last_selected_printer == 0) {
                    ...
                    sel_printers_count = list_printer->GetSelections(
                        sel_printers);
                }
            }
            if (sel_printers[0] != 0) {
                for (int i = 0; i < sel_printers_count; i++) {
                    const std::string &printer_name = list_printer->get_data(
                        sel_printers[i]);
                    ...
                    materials->filter_presets(
                        printer, EMPTY, EMPTY, [this](const Preset *p) {
                            const std::string &type = this->materials
                                                          ->get_type(p);
                ...
                materials->filter_presets(
                    nullptr, EMPTY, EMPTY, [this](const Preset *p) {
    ...
    if (sel_type != sel_type_prev) {
        ...
        // but the number of vendors is going to be very small this shouldn't
        // be a problem.
        ...
        if (sel_printers_count != 0 && sel_type != wxNOT_FOUND) {
            ...
            for (int i = 0; i < sel_printers_count; i++) {
                ...
                materials->filter_presets(printer, type, EMPTY,
                                          [this](const Preset *p) {
                                              const std::string &vendor =
                                                  this->materials->get_vendor(
                                                      p);
                                              if (list_vendor->find(vendor) ==
                                                  wxNOT_FOUND) {
    ...
    if (sel_vendor != sel_vendor_prev) {
        ...
        if (sel_printers_count != 0 && sel_type != wxNOT_FOUND &&
            ...
            for (int i = 0; i < sel_printers_count; i++) {
                ...
                materials->filter_presets(
                    printer, type, vendor, [this, &to_list](const Preset *p) {
                        const std::string &section =
                            materials->appconfig_section();
                        bool checked = wizard_p()->appconfig_new.has(section,
                                                                     p->name);
                        ...
                        if (cur_i == wxNOT_FOUND) {
                            cur_i = list_profile->append(
                                p->alias + (materials->get_omnipresent(p) ?
                                                "" :
                                                " *"),
                                &p->alias);
                            to_list.emplace_back(p->alias,
                                                 materials->get_omnipresent(p),
                                                 checked);
                        } else {
                        ...
                        if (checked && !was_checked)
                            wizard_p()->update_presets_in_config(section,
                                                                 p->alias,
                                                                 true);
                        else if (!checked && was_checked)
                            wizard_p()->appconfig_new.set(section, p->name,
                                                          "1");
                    });
            }
...
void PageMaterials::sort_list_data(StringList *list,
                                   bool        add_All_item,
                                   bool        material_type_ordering)
{
    // get data from list
    // sort data
    // first should be <all>
    ...
    if (material_type_ordering) {
        ...
        for (size_t vals = 0; vals < enum_values.size(); vals++) {
            for (size_t profs = end_of_sorted; profs < other_profiles.size();
                 profs++) {
                // find instead compare because PET vs PETG
                if (other_profiles[profs].get().find(enum_values[vals]) !=
                    std::string::npos) {
                    // swap
                    if (profs != end_of_sorted) {
                        std::reference_wrapper<const std::string> aux =
        ...
        std::sort(prusa_profiles.begin(), prusa_profiles.end(),
                  [](std::reference_wrapper<const std::string> a,
                     std::reference_wrapper<const std::string> b) {
                      return a.get() < b.get();
                  });
    ...
    list->Clear();
    if (add_All_item) list->append(_L("(All)"), &EMPTY);
...
void PageMaterials::sort_list_data(PresetList                          *list,
                                   const std::vector<ProfilePrintData> &data)
{
    // sort data
    // then prusa profiles
    // then the rest
    ...
    std::sort(prusa_profiles.begin(), prusa_profiles.end(),
              [](ProfilePrintData a, ProfilePrintData b) {
    ...
    for (size_t i = 0; i < prusa_profiles.size(); ++i) {
        list->append(std::string(prusa_profiles[i].name) +
                         (prusa_profiles[i].omnipresent ? "" : " *"),
    ...
    for (size_t i = 0; i < other_profiles.size(); ++i) {
        list->append(std::string(other_profiles[i].name) +
                         (other_profiles[i].omnipresent ? "" : " *"),
...
void PageMaterials::select_material(int i)
{
    ...
    wizard_p()->update_presets_in_config(materials->appconfig_section(),
                                         alias_key, checked);
}
...
PageCustom::PageCustom(ConfigWizard *parent)
    : ConfigWizardPage(parent,
                       _L("Custom Printer Setup"),
                       _L("Custom Printer"))
{
    cb_custom       = new wxCheckBox(this, wxID_ANY,
                                     _L("Define a custom printer profile"));
    ...
    auto *label2 = new wxStaticText(this, wxID_ANY,
                                    _L("Manage User Print Profiles: "));
    ...
    for (const auto &file : fs::directory_iterator(printer_dir))
        tempBuff.push_back(file.path().filename().string().substr(
    ...
    plistSelect = new wxCheckListBox(this, wxID_ANY, wxDefaultPosition,
                                     wxDefaultSize, tsize, plistNames, 0);
    ...
    delBtn->Bind(wxEVT_BUTTON, [this, tsize, printer_dir, plistNames,
                                tempBuff](wxCommandEvent &event) {
        bool check = false;
        ...
        if (check == false) {
            ...
            wxMessageDialog
                msg(this,
                    _L("In order for the requested changes to be made, you "
                       "must relaunch the program. All unsaved changes will "
                       "be lost. Do you want to quit?"),
                    _L("Close to save changes?"), wxYES_NO);

            if (5104 == msg.ShowModal()) {
            } else {
                for (int i = 0; i < tsize; i++) {
                    if (plistSelect->IsChecked(i)) {
                        std::string temp = printer_dir.string() + "\\" +
                                           std::string(plistNames[i].mb_str(
                                               wxConvUTF8)) +
                                           ".ini";
                        std::string temp2 = printer_dir.string() + "/" +
                                            std::string(plistNames[i].mb_str(
                                                wxConvUTF8)) +
                                            ".ini";
                        fs::path path_temp  = temp;
                        fs::path path_temp2 = temp2;
                        fs::remove(path_temp);
                        fs::remove(path_temp2);
                    }
                }
                exit(0);
            }
    ...
    , preset_update(true)
{
    ...
    auto *box_slic3r = new wxCheckBox(this, wxID_ANY,
                                      _L("Check for application updates"));
    box_slic3r->SetValue(app_config->get("notify_release") != "none");
    append(box_slic3r);
    append_text(wxString::Format(
        _L("If enabled, %s checks for new application versions online. When "
           "a new version becomes available, "
           "a notification is displayed at the next application startup "
           "(never during program usage). "
           "This is only a notification mechanisms, no automatic "
    ...
    auto *box_presets =
        new wxCheckBox(this, wxID_ANY,
                       _L("Update built-in Presets automatically"));
    box_presets->SetValue(app_config->get("preset_update") == "1");
    append(box_presets);
    append_text(wxString::Format(
        _L("If enabled, %s downloads updates of built-in system presets in "
           "the background."
           "These updates are downloaded into a separate temporary location."
           "When a new preset version becomes available it is offered at "
           "application startup."),
        SLIC3R_APP_NAME));
    const auto text_bold = _L(
        "Updates are never applied without user's consent and never "
        "overwrite user's customized settings.");
    auto *label_bold = new wxStaticText(this, wxID_ANY, text_bold);
    label_bold->SetFont(boldfont);
    label_bold->Wrap(WRAP_WIDTH);
    append(label_bold);
    append_text(_L("Additionally a backup snapshot of the whole "
                   "configuration is created before an update is applied."));

    box_slic3r->Bind(wxEVT_CHECKBOX, [this](wxCommandEvent &event) {
        this->version_check = event.IsChecked();
    });
    box_presets->Bind(wxEVT_CHECKBOX, [this](wxCommandEvent &event) {
        this->preset_update = event.IsChecked();
    });
}
    ...
    , full_pathnames(false)
{
    auto *box_pathnames =
        new wxCheckBox(this, wxID_ANY,
                       _L("Export full pathnames of models and parts sources "
                          "into 3mf and amf files"));
    box_pathnames->SetValue(
        wxGetApp().app_config->get("export_sources_full_pathnames") == "1");
    append(box_pathnames);
    append_text(_L("If enabled, allows the Reload from disk command to "
                   "automatically find and load the files when invoked.\n"
                   "If not enabled, the Reload from disk command will ask to "
                   "select each file using an open file dialog."));

    box_pathnames->Bind(wxEVT_CHECKBOX, [this](wxCommandEvent &event) {
        this->full_pathnames = event.IsChecked();
    });
}
...
PageFilesAssociation::PageFilesAssociation(ConfigWizard *parent)
    : ConfigWizardPage(parent,
                       ...
                       _L("Files association"))
{
    cb_3mf = new wxCheckBox(this, wxID_ANY,
                            _L("Associate .3mf files to PrusaSlicer"));
    cb_stl = new wxCheckBox(this, wxID_ANY,
                            _L("Associate .stl files to PrusaSlicer"));
    //    cb_gcode = new wxCheckBox(this, wxID_ANY, _L("Associate .gcode files
    //    to PrusaSlicer G-code Viewer"));
    ...
    : ConfigWizardPage(parent, _L("View mode"), _L("View mode"))
{
    append_text(_L(
        ...
        "Advanced, and Expert.\n"
        "The Simple mode shows only the most frequently used settings "
        "relevant for regular 3D printing. "
    ...
    : ConfigWizardPage(parent, _L("Other Vendors"), _L("Other Vendors"))
{
    ...
    append_text(wxString::Format(_L("Pick another vendor supported by %s"),
                                 SLIC3R_APP_NAME) +
                ":");
    ...
    for (const auto &pair : wizard_p()->bundles) {
        ...
        if (enabled) {
            ...
            wxCHECK_RET(
                pages != wizard_p()->pages_3rdparty.end(),
                "Internal error: 3rd party vendor printers page not created");

            for (PagePrinters *page :
                 {pages->second.first, pages->second.second})
    ...
    , gcode_picker(nullptr)
{
    ...
    for (const auto &label : gcode_opt.enum_labels) { choices.Add(label); }

    gcode_picker = new wxChoice(this, wxID_ANY, wxDefaultPosition,
    ...
    if (gcode_opt.default_value) {
        needle = std::find(enum_values.cbegin(), enum_values.cend(),
                           gcode_opt.default_value->serialize());
    }
...
void PageFirmware::apply_custom_config(DynamicPrintConfig &config)
{
    ...
    if (sel >= 0 && (size_t) sel < gcode_opt.enum_labels.size()) {
        auto *opt = new ConfigOptionEnum<GCodeFlavor>(
            static_cast<GCodeFlavor>(sel));
    ...
    , shape_panel(new BedShapePanel(this))
{
    ...
    shape_panel
        ->build_panel(*wizard_p()->custom_config->option<ConfigOptionPoints>(
                          "bed_shape"),
                      *wizard_p()->custom_config->option<ConfigOptionString>(
                          "bed_custom_texture"),
                      *wizard_p()->custom_config->option<ConfigOptionString>(
                          "bed_custom_model"));
...
void PageBedShape::apply_custom_config(DynamicPrintConfig &config)
{
    ...
    config.set_key_value("bed_custom_texture",
                         new ConfigOptionString(custom_texture));
    config.set_key_value("bed_custom_model",
                         new ConfigOptionString(custom_model));
}
...
public:
    DiamTextCtrl(wxWindow *parent)
    {
        ...
        Create(parent, wxID_ANY, wxEmptyString, wxDefaultPosition,
               wxSize(Field::def_width_thinner() * wxGetApp().em_unit(),
                      wxDefaultCoord),
               style);
        wxGetApp().UpdateDarkUI(this);
    }
...
PageDiameters::PageDiameters(ConfigWizard *parent)
    : ConfigWizardPage(parent,
                       _L("Filament and Nozzle Diameters"),
                       _L("Print Diameters"),
                       1)
    , diam_nozzle(new DiamTextCtrl(this))
    , diam_filam(new DiamTextCtrl(this))
{
    auto *default_nozzle = print_config_def.get("nozzle_diameter")
                               ->get_default_value<ConfigOptionFloats>();
    wxString value = double_to_string(default_nozzle != nullptr &&
                                              default_nozzle->size() > 0 ?
                                          default_nozzle->get_at(0) :
                                          0.5);
    diam_nozzle->SetValue(value);

    auto *default_filam = print_config_def.get("filament_diameter")
                              ->get_default_value<ConfigOptionFloats>();
    value = double_to_string(default_filam != nullptr &&
                                     default_filam->size() > 0 ?
                                 default_filam->get_at(0) :
                                 3.0);
    diam_filam->SetValue(value);

    diam_nozzle->Bind(
        wxEVT_KILL_FOCUS,
        [this](wxFocusEvent &e) { focus_event(e, diam_nozzle, 0.5); },
        diam_nozzle->GetId());
    diam_filam->Bind(
        wxEVT_KILL_FOCUS,
        [this](wxFocusEvent &e) { focus_event(e, diam_filam, 3.0); },
        diam_filam->GetId());

    append_text(_L("Enter the diameter of your printer's hot end nozzle."));

    auto *sizer_nozzle = new wxFlexGridSizer(3, 5, 5);
    auto *text_nozzle  = new wxStaticText(this, wxID_ANY,
                                          _L("Nozzle Diameter:"));
    ...
    append_text(
        _L("Good precision is required, so use a caliper and do multiple "
           "measurements along the filament, then compute the average."));

    auto *sizer_filam = new wxFlexGridSizer(3, 5, 5);
    auto *text_filam  = new wxStaticText(this, wxID_ANY,
                                         _L("Filament Diameter:"));
...
void PageDiameters::apply_custom_config(DynamicPrintConfig &config)
{
    ...
    auto set_extrusion_width = [&config, opt_nozzle](const char *key,
                                                     double      dmr) {
        char buf[64]; // locales don't matter here (sprintf/atof)
        sprintf(buf, "%.2lf", dmr * opt_nozzle->values.front() / 0.4);
        config.set_key_value(key, new ConfigOptionFloatOrPercent(atof(buf),
                                                                 false));
    };
...
PageTemperatures::PageTemperatures(ConfigWizard *parent)
    : ConfigWizardPage(parent,
                       _L("Nozzle and Bed Temperatures"),
                       _L("Temperatures"),
                       1)
    ...
    , spin_bed(new SpinCtrlDouble(this))
{
    ...
    spin_extr->SetValue(default_extr != nullptr && default_extr->size() > 0 ?
                            default_extr->get_at(0) :
                            200);
    ...
    spin_bed->SetValue(default_bed != nullptr && default_bed->size() > 0 ?
                           default_bed->get_at(0) :
                           0);

    append_text(
        _L("Enter the temperature needed for extruding your filament."));
    append_text(_L("A rule of thumb is 160 to 230 °C for PLA, and 215 to 250 "
                   "°C for ABS."));

    auto *sizer_extr = new wxFlexGridSizer(3, 5, 5);
    auto *text_extr  = new wxStaticText(this, wxID_ANY,
                                        _L("Extrusion Temperature:"));
    ...
    append_text(_L("Enter the bed temperature needed for getting your "
                   "filament to stick to your heated bed."));
    append_text(_L("A rule of thumb is 60 °C for PLA and 110 °C for ABS. "
                   "Leave zero if you have no heated bed."));
...
{
#ifndef __WXOSX__
    SetDoubleBuffered(true); // SetDoubleBuffered exists on Win and Linux/GTK,
                             // but is missing on OSX
#endif                       //__WXOSX__
...
void ConfigWizardIndex::on_paint(wxPaintEvent &evt)
{
    ...
    for (size_t i = 0; i < items.size(); i++) {
        ...
        if (i == item_active ||
            (item_hover >= 0 && i == (size_t) item_hover)) {
...
void ConfigWizardIndex::on_mouse_move(wxMouseEvent &evt)
{
    ...
    if (item_hover_new < ssize_t(items.size()) &&
        item_hover_new != item_hover) {
...
void Materials::push(const Preset *preset)
{
    ...
    types.insert(technology & T_FFF ? Materials::get_filament_type(preset) :
...
{
    return (technology & T_FFF) ? AppConfig::SECTION_FILAMENTS :
                                  AppConfig::SECTION_MATERIALS;
}

const std::string &Materials::get_type(const Preset *preset) const
{
    return (technology & T_FFF) ? get_filament_type(preset) :
                                  get_material_type(preset);
}

const std::string &Materials::get_vendor(const Preset *preset) const
{
    return (technology & T_FFF) ? get_filament_vendor(preset) :
                                  get_material_vendor(preset);
}

const std::string &Materials::get_filament_type(const Preset *preset)
{
    const auto *opt = preset->config.opt<ConfigOptionStrings>(
        "filament_type");
...
{
    const auto *opt = preset->config.opt<ConfigOptionString>(
        "filament_vendor");
...
static const std::unordered_map<std::string,
    ...
    legacy_preset_map{{
        ...
        {"Original Prusa i3 MK2 MM Single Mode.ini",
         std::make_pair("MK2SMM", "0.4")},
        {"Original Prusa i3 MK2 MM Single Mode 0.6 nozzle.ini",
         std::make_pair("MK2SMM", "0.6")},
        {"Original Prusa i3 MK2 MultiMaterial.ini",
         std::make_pair("MK2SMM", "0.4")},
        {"Original Prusa i3 MK2 MultiMaterial 0.6 nozzle.ini",
         std::make_pair("MK2SMM", "0.6")},
        {"Original Prusa i3 MK2 0.25 nozzle.ini",
         std::make_pair("MK2S", "0.25")},
        {"Original Prusa i3 MK2 0.6 nozzle.ini",
         std::make_pair("MK2S", "0.6")},
...
void ConfigWizard::priv::load_pages()
{
    ...
    // Printers
    if (!only_sla_mode) index->add_page(page_fff);
    ...
    if (!only_sla_mode) {
        ...
        for (const auto &pages : pages_3rdparty) {
            for (PagePrinters *page :
                 {pages.second.first, pages.second.second})
                if (page && page->install) index->add_page(page);
        }
    ...
    index->go_to(
        former_active); // Will restore the active item/page if possible

    q->Layout();
    // This Refresh() is needed to avoid ugly artifacts after printer
    // selection, when no one vendor was selected from the very beginnig
    q->Refresh();
}
...
void ConfigWizard::priv::init_dialog_size()
{
    ...
    wxRect     window_rect(disp_rect.x + disp_rect.width / 20,
                           disp_rect.y + disp_rect.height / 20,
                           9 * disp_rect.width / 10, 9 * disp_rect.height / 10);

    const int width_hint =
        index->GetSize().GetWidth() +
        std::max(
            90 * em(),
            (only_sla_mode ? page_msla->get_width() : page_fff->get_width()) +
...
void ConfigWizard::priv::load_vendors()
{
    ...
    // Load up the set of vendors / models / variants the user has had enabled
    // up till now
    AppConfig *app_config = wxGetApp().app_config;
    if (!app_config->legacy_datadir()) {
        appconfig_new.set_vendors(*app_config);
    } else {
        // In case of legacy datadir, try to guess the preference based on the
        // printer preset files that are present
        const auto printer_dir = fs::path(Slic3r::data_dir()) / "printer";
        for (auto &dir_entry :
             boost::filesystem::directory_iterator(printer_dir))
            if (Slic3r::is_ini_file(dir_entry)) {
                ...
                appconfig_new.set_variant("PrusaResearch", model, variant,
                                          true);
            }
    }
    ...
    // Copy installed filaments and SLA material names from app_config to
    // appconfig_new while resolving current names of profiles, which were
    // renamed in the meantime.
    for (PrinterTechnology technology : {ptFFF, ptSLA}) {
        const std::string &section_name = (technology == ptFFF) ?
                                              AppConfig::SECTION_FILAMENTS :
                                              AppConfig::SECTION_MATERIALS;
        std::map<std::string, std::string> section_new;
        if (app_config->has_section(section_name)) {
            const std::map<std::string, std::string> &section_old =
            ...
            for (const auto &material_name_and_installed : section_old)
                if (material_name_and_installed.second == "1") {
                    ...
                    const std::string &material_name =
                        material_name_and_installed.first;
                    for (auto &bundle : bundles) {
                        const PresetCollection &materials =
                            bundle.second.preset_bundle->materials(technology);
                        const Preset *preset = materials.find_preset(
                            material_name);
                        if (preset == nullptr) {
                            // Not found. Maybe the material preset is there,
                            // bu it was was renamed?
                            const std::string *new_name =
                                materials.get_preset_name_renamed(
                                    material_name);
                            if (new_name != nullptr)
                    ...
                    if (num_found == 0)
                        BOOST_LOG_TRIVIAL(error)
                            << boost::format(
                                   "Profile %1% was not found in installed "
                                   "vendor Preset Bundles.") %
                                   material_name;
                    else if (num_found > 1)
                        BOOST_LOG_TRIVIAL(error)
                            << boost::format("Profile %1% was found in %2% "
                                             "vendor Preset Bundles.") %
                                   material_name % num_found;
                }
        }
...
{
    const int proportion = (page->shortname == _L("Filaments")) ||
                                   (page->shortname == _L("SLA Materials")) ?
                               1 :
                               0;
...
void ConfigWizard::priv::create_3rdparty_pages()
{
    for (const auto &pair : bundles) {
        ...
        if (is_fff_technology) {
            pageFFF = new PagePrinters(q,
                                       vendor->name + " " +
                                           _L("FFF Technology Printers"),
                                       vendor->name + " FFF", *vendor, 1,
                                       T_FFF);
            add_page(pageFFF);
        }

        if (is_sla_technology) {
            pageSLA = new PagePrinters(q,
                                       vendor->name + " " +
                                           _L("SLA Technology Printers"),
                                       vendor->name + " MSLA", *vendor, 1,
                                       T_SLA);
            add_page(pageSLA);
        }
...
void ConfigWizard::priv::set_run_reason(RunReason run_reason)
{
    ...
    for (auto &page : all_pages) { page->set_run_reason(run_reason); }
}
    ...
    if (any_fff_selected && (technology & T_FFF)) {
        ...
        for (const auto &pair : bundles) {
            for (const auto &filament : pair.second.preset_bundle->filaments) {
                ...
                for (const auto &printer :
                     pair.second.preset_bundle->printers) {
                    if (!printer.is_visible ||
                        printer.printer_technology() != ptFFF)
                        continue;
                    // Filter out inapplicable printers
                    if (is_compatible_with_printer(
                            PresetWithVendorProfile(filament, filament.vendor),
                            PresetWithVendorProfile(printer,
                                                    printer.vendor))) {
                        if (!filaments.containts(&filament)) {
                            filaments.push(&filament);
                            if (!filament.alias.empty())
                                aliases_fff[filament.alias].insert(
                                    filament.name);
                        }
        ...
        for (const auto &preset : filaments.presets) {
            const auto filter =
                [preset](const std::pair<std::string, size_t> element) {
                    return preset->alias == element.first;
                };
            if (std::find_if(filaments.compatibility_counter.begin(),
                             filaments.compatibility_counter.end(), filter) !=
            ...
            for (const auto &printer : filaments.printers) {
                ...
                for (size_t i = 0;
                     i < idx_with_same_alias.size() && !compatible; ++i) {
                    const Preset &prst = *(
                        filaments.presets[idx_with_same_alias[i]]);
                    const Preset &prntr = *printer;
                    if (is_compatible_with_printer(
                            PresetWithVendorProfile(prst, prst.vendor),
                            PresetWithVendorProfile(prntr, prntr.vendor))) {
                        compatible = true;
                        break;
                    }
                }
                if (compatible) counter++;
            }
            filaments.compatibility_counter.emplace_back(preset->alias,
                                                         counter);
        }
    }
    ...
    if (any_sla_selected && (technology & T_SLA)) {
        ...
        for (const auto &pair : bundles) {
            for (const auto &material :
                 pair.second.preset_bundle->sla_materials) {
                // Check if material is already added
                if (sla_materials.containts(&material)) continue;
                // Iterate printers in all bundles
                // For now, we only allow the profiles to be compatible with
                // another profiles inside the same bundle.
                for (const auto &printer :
                     pair.second.preset_bundle->printers) {
                    if (!printer.is_visible ||
                        printer.printer_technology() != ptSLA)
                        continue;
                    // Filter out inapplicable printers
                    if (is_compatible_with_printer(
                            PresetWithVendorProfile(material, nullptr),
                        ...
                        if (!sla_materials.containts(&material)) {
                            ...
                            if (!material.alias.empty())
                                aliases_sla[material.alias].insert(
                                    material.name);
                        }
        ...
        for (const auto &preset : sla_materials.presets) {
            ...
            if (std::find_if(sla_materials.compatibility_counter.begin(),
            ...
            for (const auto &printer : sla_materials.printers) {
                ...
                for (size_t i = 0;
                     i < idx_with_same_alias.size() && !compatible; ++i) {
                    const Preset &prst = *(
                        sla_materials.presets[idx_with_same_alias[i]]);
                    const Preset &prntr = *printer;
                    if (is_compatible_with_printer(
                            PresetWithVendorProfile(prst, prst.vendor),
                            PresetWithVendorProfile(prntr, prntr.vendor))) {
            ...
            sla_materials.compatibility_counter.emplace_back(preset->alias,
                                                             counter);
        }
    }
}
    ...
    for (auto &pair : bundles) {
        ...
        for (auto &preset : pair.second.preset_bundle->printers) {
            if (preset.config.opt_string("printer_model") == evt.model_id &&
                preset.config.opt_string("printer_variant") ==
                    evt.variant_name) {
                preset.is_visible = evt.enable;
            }
        }

        // When a printer model is picked, but there is no material installed
        // compatible with this printer model, install default materials for
        // selected printer model silently.
...
void ConfigWizard::priv::select_default_materials_for_printer_model(
    const VendorProfile::PrinterModel &printer_model, Technology technology)
{
    PageMaterials *page_materials = technology & T_FFF ? page_filaments :
                                                         page_sla_materials;
    for (const std::string &material : printer_model.default_materials)
        appconfig_new.set(page_materials->materials->appconfig_section(),
                          material, "1");
}
    ...
    const std::set<const VendorProfile::PrinterModel *> &printer_models)
{
    PageMaterials     *page_materials = technology & T_FFF ? page_filaments :
                                                             page_sla_materials;
    const std::string &appconfig_section = page_materials->materials
                                               ->appconfig_section();

    // Following block was unnecessary. Its enough to iterate printer_models
    // once. Not for every vendor printer page. Filament is selected on same
    // page for all printers of same technology.
    /*
    auto select_default_materials_for_printer_page = [this, appconfig_section,
    printer_models, technology](PagePrinters *page_printers, Technology
    technology)
    {
        const std::string vendor_id = page_printers->get_vendor_id();
        for (auto& pair : bundles)
            if (pair.first == vendor_id)
                for (const VendorProfile::PrinterModel *printer_model :
    printer_models) for (const std::string &material :
    printer_model->default_materials) appconfig_new.set(appconfig_section,
    material, "1");
    };
    ...
    {
        page_printers = technology & T_FFF ? printer.second.first :
    printer.second.second; if (page_printers)
            select_default_materials_for_printer_page(page_printers,
    technology);
    }
    */

    // Iterate printer_models and select default materials. If none available
    // -> msg to user.
    ...
    for (const VendorProfile::PrinterModel *printer_model : printer_models) {
        if (printer_model->default_materials.empty()) {
            ...
            for (const std::string &material :
                 printer_model->default_materials)
    ...
    if (!models_without_default.empty()) {
        ...
        std::string message =
            (technology & T_FFF ?
                 GUI::format(_L("Following printer profiles has no default "
                                "filament: %1%Please select one manually."),
                             printer_names) :
                 GUI::format(_L("Following printer profiles has no default "
                                "material: %1%Please select one manually."),
                             printer_names));
    ...
    ((technology & T_FFF) ? page_filaments : page_sla_materials)
        ->reload_presets();
}

void ConfigWizard::priv::on_3rdparty_install(const VendorProfile *vendor,
                                             bool                 install)
{
    auto it = pages_3rdparty.find(vendor->id);
    wxCHECK_RET(
        it != pages_3rdparty.end(),
        "Internal error: GUI page not found for 3rd party vendor profile");

    for (PagePrinters *page : {it->second.first, it->second.second})
        if (page) {
            if (page->install && !install) page->select_all(false);
            page->install = install;
            // if some 3rd vendor is selected, select first printer for them
            if (install) page->printer_pickers[0]->select_one(0, true);
            page->Layout();
        }
...
bool ConfigWizard::priv::on_bnt_finish()
{
    ...
    if (any_fff_selected) page_filaments->reload_presets();
    if (any_sla_selected) page_sla_materials->reload_presets();

    // theres no need to check that filament is selected if we have only
    // custom printer
...
// This allmighty method verifies, whether there is at least a single
// compatible filament or SLA material installed for each Printer preset of
// each Printer Model installed.
//
// In case only_for_model_id is set, then the test is done for that particular
// printer model only, and the default materials are installed silently.
...
// Return false if there were some Printer Models with missing materials,
// independent from whether the defaults were installed for these respective
// Printer Models or not.
bool ConfigWizard::priv::check_and_install_missing_materials(
    Technology technology, const std::string &only_for_model_id)
{
    // Walk over all installed Printer presets and verify whether there is a
    // filament or SLA material profile installed at the same PresetBundle,
    // which is compatible with it.
    const auto printer_models_missing_materials =
        [this, only_for_model_id](PrinterTechnology  technology,
                                  const std::string &section) {
            const std::map<std::string, std::string> &appconfig_presets =
                appconfig_new.has_section(section) ?
                    appconfig_new.get_section(section) :
                    std::map<std::string, std::string>();
            std::set<const VendorProfile::PrinterModel *>
                printer_models_without_material;
            for (const auto &pair : bundles) {
                const PresetCollection &materials =
                    pair.second.preset_bundle->materials(technology);
                for (const auto &printer :
                     pair.second.preset_bundle->printers) {
                    if (printer.is_visible &&
                        printer.printer_technology() == technology) {
                        const VendorProfile::PrinterModel *printer_model =
                            PresetUtils::system_printer_model(printer);
                        assert(printer_model != nullptr);
                        if ((only_for_model_id.empty() ||
                             only_for_model_id == printer_model->id) &&
                            printer_models_without_material.find(
                                printer_model) ==
                                printer_models_without_material.end()) {
                            bool has_material = false;
                            for (const auto &preset : appconfig_presets) {
                                if (preset.second == "1") {
                                    const Preset *material =
                                        materials.find_preset(preset.first,
                                                              false);
                                    if (material != nullptr &&
                                        is_compatible_with_printer(
                                            PresetWithVendorProfile(*material,
                                                                    nullptr),
                                            PresetWithVendorProfile(printer,
                                                                    nullptr))) {
                                        has_material = true;
                            ...
                            if (!has_material)
                                printer_models_without_material.insert(
                                    printer_model);
                        }
                    }
                }
            }
            assert(printer_models_without_material.empty() ||
                   only_for_model_id.empty() ||
                   only_for_model_id ==
                       (*printer_models_without_material.begin())->id);
            return printer_models_without_material;
        };

    const auto ask_and_select_default_materials =
        [this](const wxString &message,
               const std::set<const VendorProfile::PrinterModel *>
                         &printer_models,
               Technology technology) {
            // wxMessageDialog msg(q, message, _L("Notice"), wxYES_NO);
            MessageDialog msg(q, message, _L("Notice"), wxYES_NO);
            if (msg.ShowModal() == wxID_YES)
                select_default_materials_for_printer_models(technology,
                                                            printer_models);
        };

    const auto printer_model_list =
        [](const std::set<const VendorProfile::PrinterModel *> &printer_models)
        -> wxString {
        wxString out;
        for (const VendorProfile::PrinterModel *printer_model :
    ...
    if (any_fff_selected && (technology & T_FFF)) {
        std::set<const VendorProfile::PrinterModel *>
            printer_models_without_material =
                printer_models_missing_materials(ptFFF,
                                                 AppConfig::SECTION_FILAMENTS);
        if (!printer_models_without_material.empty()) {
            if (only_for_model_id.empty())
                ask_and_select_default_materials(
                    _L("The following FFF printer models have no filament "
                       "selected:") +
                        "\n\n\t" +
                        printer_model_list(printer_models_without_material) +
                        "\n\n\t" +
                        _L("Do you want to select default filaments for "
                           "these FFF printer models?"),
                    printer_models_without_material, T_FFF);
            else
                select_default_materials_for_printer_model(
                    **printer_models_without_material.begin(), T_FFF);
            return false;
        }
    }
    ...
    if (any_sla_selected && (technology & T_SLA)) {
        std::set<const VendorProfile::PrinterModel *>
            printer_models_without_material =
                printer_models_missing_materials(ptSLA,
                                                 AppConfig::SECTION_MATERIALS);
        if (!printer_models_without_material.empty()) {
            if (only_for_model_id.empty())
                ask_and_select_default_materials(
                    _L("The following SLA printer models have no materials "
                       "selected:") +
                        "\n\n\t" +
                        printer_model_list(printer_models_without_material) +
                        "\n\n\t" +
                        _L("Do you want to select default SLA materials for "
                           "these printer models?"),
                    printer_models_without_material, T_SLA);
            else
                select_default_materials_for_printer_model(
                    **printer_models_without_material.begin(), T_SLA);
            return false;
        }
    }
...
static std::set<std::string> get_new_added_presets(
    const std::map<std::string, std::string> &old_data,
    const std::map<std::string, std::string> &new_data)
{
    auto get_aliases = [](const std::map<std::string, std::string> &data) {
        ...
        for (auto item : data) {
            ...
            old_aliases.emplace(
                pos == std::string::npos ? name : name.substr(0, pos - 1));
        }
        return old_aliases;
    };
    ...
    std::set_difference(new_aliases.begin(), new_aliases.end(),
                        old_aliases.begin(), old_aliases.end(),
                        std::inserter(diff, diff.begin()));

    return diff;
}

static std::string get_first_added_preset(
    const std::map<std::string, std::string> &old_data,
    const std::map<std::string, std::string> &new_data)
{
    std::set<std::string> diff = get_new_added_presets(old_data, new_data);
    if (diff.empty()) return std::string();
    return *diff.begin();
}
                                      ...
                                      bool &apply_keeped_changes)
{
    ...
    bool suppress_sla_printer = model_has_multi_part_objects(
        wxGetApp().model());
    PrinterTechnology preferred_pt = ptAny;
    auto              get_preferred_printer_technology =
        [enabled_vendors, suppress_sla_printer](const std::string &bundle_name,
                                                const Bundle      &bundle) {
            ...
            if (config != enabled_vendors.end()) {
                for (const auto &model : bundle.vendor_profile->models) {
                    if (const auto model_it = config->second.find(model.id);
                        model_it != config->second.end() &&
                        model_it->second.size() > 0) {
                        if (pt == ptAny) pt = model.technology;
                        // if preferred printer model has SLA printer technology
                        // it's important to check the model for multypart state
    ...
    if (preferred_pt = get_preferred_printer_technology("PrusaResearch",
                                                        bundles.prusa_bundle());
        preferred_pt == ptAny ||
        (preferred_pt == ptSLA && suppress_sla_printer)) {
        for (const auto &bundle : bundles) {
            if (bundle.second.is_prusa_bundle) { continue; }
            if (PrinterTechnology pt =
                    get_preferred_printer_technology(bundle.first,
                                                     bundle.second);
                pt == ptAny)
                continue;
    ...
    for (const auto &pair : bundles) {
        ...
        if (pair.second.is_prusa_bundle) {
            // Always install Prusa bundle, because it has a lot of
            // filaments/materials likely to be referenced by other profiles.
    ...
    if (!check_unsaved_preset_changes)
        if ((check_unsaved_preset_changes = install_bundles.size() > 0))
            header = _L_PLURAL("A new vendor was installed and one of its "
                               "printers will be activated",
                               "New vendors were installed and one of theirs "
                               "printers will be activated",
    ...
    // Decide whether to create snapshot based on run_reason and the reset
    // profile checkbox
    bool             snapshot        = true;
    ...
    if (snapshot &&
        !take_config_snapshot_cancel_on_error(
            *app_config, snapshot_reason, "",
            _u8L("Do you want to continue changing the configuration?")))
        return false;

    if (check_unsaved_preset_changes &&
        !wxGetApp()
             .check_and_keep_current_preset_changes(caption, header, act_btns,
    ...
    if (install_bundles.size() > 0) {
        ...
        BOOST_LOG_TRIVIAL(info)
            << "No bundles need to be installed from resources";
    }
    ...
    auto        get_preferred_printer_model =
        [enabled_vendors, enabled_vendors_old,
         preferred_pt](const std::string &bundle_name, const Bundle &bundle,
                       std::string &variant) {
            const auto config = enabled_vendors.find(bundle_name);
            if (config == enabled_vendors.end()) return std::string();
            for (const auto &model : bundle.vendor_profile->models) {
                if (const auto model_it = config->second.find(model.id);
                    model_it != config->second.end() &&
                    model_it->second.size() > 0 &&
                    ...
                    const auto config_old = enabled_vendors_old.find(
                        bundle_name);
                    ...
                    if (model_it_old == config_old->second.end())
                        ...
                        for (const auto &var : model_it->second)
                            if (model_it_old->second.find(var) ==
                                model_it_old->second.end()) {
                                variant = var;
                                return model.id;
                            }
                    }
            ...
            if (!variant.empty()) variant.clear();
            return std::string();
        };
    // Prusa printers are considered first, then 3rd party.
    if (preferred_model = get_preferred_printer_model("PrusaResearch",
                                                      bundles.prusa_bundle(),
                                                      preferred_variant);
        ...
        for (const auto &bundle : bundles) {
            if (bundle.second.is_prusa_bundle) { continue; }
            if (preferred_model =
                    get_preferred_printer_model(bundle.first, bundle.second,
                                                preferred_variant);
                !preferred_model.empty())
                break;
        }
    ...
    if (!check_unsaved_preset_changes) {
        if ((check_unsaved_preset_changes = !preferred_model.empty())) {
            header = _L(
                "A new Printer was installed and it will be activated.");
            if (!wxGetApp().check_and_keep_current_preset_changes(
                    caption, header, act_btns, &apply_keeped_changes))
                return false;
        } else if ((check_unsaved_preset_changes = enabled_vendors_old !=
                                                   enabled_vendors)) {
    ...
    auto        get_first_added_material_preset =
        [this, app_config](const std::string &section_name,
                           std::string       &first_added_preset) {
            if (appconfig_new.has_section(section_name)) {
                // get first of new added preset names
                const std::map<std::string, std::string> &old_presets =
                    app_config->has_section(section_name) ?
                        app_config->get_section(section_name) :
                        std::map<std::string, std::string>();
                first_added_preset =
                    get_first_added_preset(old_presets,
                                           appconfig_new.get_section(
                                               section_name));
            }
        };
    get_first_added_material_preset(AppConfig::SECTION_FILAMENTS,
                                    first_added_filament);
    get_first_added_material_preset(AppConfig::SECTION_MATERIALS,
                                    first_added_sla_material);

    // if unsaved changes was not cheched till this moment
    if (!check_unsaved_preset_changes) {
        if ((check_unsaved_preset_changes = !first_added_filament.empty() ||
                                            !first_added_sla_material.empty())) {
            header = !first_added_filament.empty() ?
                         _L("A new filament was installed and it will be "
                            "activated.") :
                         _L("A new SLA material was installed and it will be "
                            "activated.");
            if (!wxGetApp().check_and_keep_current_preset_changes(
                    caption, header, act_btns, &apply_keeped_changes))
                return false;
        } else {
            auto changed = [app_config, &appconfig_new = std::as_const(
                                            this->appconfig_new)](
                               const std::string &section_name) {
                return (app_config->has_section(section_name) ?
                            app_config->get_section(section_name) :
                            std::map<std::string, std::string>()) !=
                       appconfig_new.get_section(section_name);
            };
            bool is_filaments_changed = changed(AppConfig::SECTION_FILAMENTS);
            bool is_sla_materials_changed = changed(
                AppConfig::SECTION_MATERIALS);
            if ((check_unsaved_preset_changes = is_filaments_changed ||
                                                is_sla_materials_changed)) {
                header = is_filaments_changed ?
                             _L("Some filaments were uninstalled.") :
    ...
    for (const std::string &section_name :
         {AppConfig::SECTION_FILAMENTS, AppConfig::SECTION_MATERIALS})
        app_config->set_section(section_name,
                                appconfig_new.get_section(section_name));

    app_config->set_vendors(appconfig_new);

    app_config->set("notify_release",
                    page_update->version_check ? "all" : "none");
    app_config->set("preset_update", page_update->preset_update ? "1" : "0");
    app_config->set("export_sources_full_pathnames",
                    page_reload_from_disk->full_pathnames ? "1" : "0");

#ifdef _WIN32
    app_config->set("associate_3mf",
                    page_files_association->associate_3mf() ? "1" : "0");
    app_config->set("associate_stl",
                    page_files_association->associate_stl() ? "1" : "0");
    //    app_config->set("associate_gcode",
    //    page_files_association->associate_gcode() ? "1" : "0");
    ...
    if (!only_sla_mode && page_custom->custom_wanted()) {
        ...
        if (!check_unsaved_preset_changes &&
            !wxGetApp().check_and_keep_current_preset_changes(
                caption,
                _L("Custom printer was installed and it will be activated."),
...
void ConfigWizard::priv::update_presets_in_config(const std::string &section,
                                                  const std::string &alias_key,
                                                  bool               add)
{
    const PresetAliases &aliases = section == AppConfig::SECTION_FILAMENTS ?
                                       aliases_fff :
                                       aliases_sla;
    ...
    if (it != aliases.end())
        for (const std::string &name : it->second) update(section, name);
}
    ...
    , p(new priv(this))
{
    ...
    // Initially we _do not_ SetScrollRate in order to figure out the overall
    // width of the Wizard  without scrolling. Later, we compare that to the
    // size of the current screen and set minimum width based on that (see below).
    ...
    p->btn_sel_all = new wxButton(this, wxID_ANY,
                                  _L("Select all standard printers"));
    ...
    p->btn_cancel = new wxButton(
        this, wxID_CANCEL,
        _L("Cancel")); // Note: The label needs to be present, otherwise we
                       // get accelerator bugs on Mac
    ...
    wxCHECK_RET(prusa_it != p->bundles.cend(),
                "Vendor PrusaResearch not found");
    const VendorProfile *vendor_prusa = prusa_it->second.vendor_profile;

    p->add_page(p->page_welcome = new PageWelcome(this));

    p->page_fff = new PagePrinters(this, _L("Prusa FFF Technology Printers"),
                                   "Prusa FFF", *vendor_prusa, 0, T_FFF);
    ...
    p->page_msla = new PagePrinters(this,
                                    _L("Prusa MSLA Technology Printers"),
                                    "Prusa MSLA", *vendor_prusa, 0, T_SLA);
    p->add_page(p->page_msla);
    if (p->only_sla_mode) { p->page_msla->is_primary_printer_page = true; }

    if (!p->only_sla_mode) {
        // Pages for 3rd party vendors
        p->create_3rdparty_pages(); // Needs to be done _before_ creating
                                    // PageVendors
    ...
    if (!p->only_sla_mode)
        p->add_page(p->page_filaments =
                        new PageMaterials(this, &p->filaments,
                                          _L("Filament Profiles Selection"),
                                          _L("Filaments"), _L("Type:")));

    p->add_page(
        p->page_sla_materials =
            new PageMaterials(this, &p->sla_materials,
                              _L("SLA Material Profiles Selection") + " ",
    ...
    p->btn_next->Bind(wxEVT_BUTTON, [this](const wxCommandEvent &) {
        ...
        if ( // Leaving the filaments or SLA materials page and
            (active_page == p->page_filaments ||
             active_page == p->page_sla_materials) &&
            // some Printer models had no filament or SLA material selected.
            !p->check_and_install_missing_materials(
                dynamic_cast<PageMaterials *>(active_page)
                    ->materials->technology))
            // In that case don't leave the page and the function above
            // queried the user whether to install default materials.
            return;
        this->p->index->go_next();
    });

    p->btn_finish->Bind(wxEVT_BUTTON, [this](const wxCommandEvent &) {
        if (p->on_bnt_finish()) this->EndModal(wxID_OK);
    });
    ...
    p->index->Bind(EVT_INDEX_PAGE, [this](const wxCommandEvent &) {
        ...
        if (is_last) p->btn_finish->SetFocus();
...
bool ConfigWizard::run(RunReason reason, StartPage start_page)
{
    BOOST_LOG_TRIVIAL(info)
        << boost::format(
               "Running ConfigWizard, reason: %1%, start_page: %2%") %
               reason % start_page;
    ...
    if (ShowModal() == wxID_OK) {
        ...
        if (!p->apply_config(app.app_config, app.preset_bundle,
                             app.preset_updater, apply_keeped_changes))
            return false;

        if (apply_keeped_changes) app.apply_keeped_preset_modifications();
...
{
    // A different naming convention is used for the Wizard on Windows & GTK
    // vs. OSX. Note: Don't call _() macro here.
    ...
    static const wxString config_wizard_name_menu = L(
...
void ConfigWizard::on_dpi_changed(const wxRect &suggested_rect)
{
    ...
    msw_buttons_rescale(this, em,
                        {wxID_APPLY, wxID_CANCEL, p->btn_sel_all->GetId(),
                         p->btn_next->GetId(), p->btn_prev->GetId()});
