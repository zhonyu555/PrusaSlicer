...
void GLCanvas3D::Labels::render(const std::vector<const ModelInstance*>& sorted_instances) const
{
    ...
    for (const Owner& owner : owners) {
        ...
        ImGui::PushStyleColor(ImGuiCol_Border, owner.selected ? ImVec4(0.26f, 0.116f, 0.118f, 1.0f) : ImVec4(0.26f, 0.116f, 0.118f, 1.0f));
...
void GLCanvas3D::SLAView::render_debug_window()
{
    ...
    for (const auto& [id, type] : m_instances_cache) {
        imgui.text_colored(ImGuiWrapper::COL_GREEN_DARK, "(" + std::to_string(id.object_id) + ", " + std::to_string(id.instance_id) + ")");
        ImGui::SameLine();
