...
CalibrationAbstractDialog::CalibrationAbstractDialog(GUI_App *app, MainFrame *mainframe, std::string name)
    : DPIDialog(NULL,
                wxID_ANY,
                wxString(SLIC3R_APP_NAME) + " - " + _(L(name)),
#if ENABLE_SCROLLABLE
                wxDefaultPosition,
                wxDefaultSize,
...
void CalibrationAbstractDialog::create(boost::filesystem::path html_path, std::string html_name, wxSize dialog_size)
{
    ...
    boost::filesystem::path full_file_path = (boost::filesystem::path(Slic3r::resources_dir()) / html_path /
                                              (into_u8(language) + "_" + html_name));
    if (language == "en") {
    ...
    public:
        ...
        {
            if (link.GetHref().StartsWith(_T("http://")) || link.GetHref().StartsWith(_T("https://")))
    ...
    html_viewer = new HtmlWindow(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxHW_SCROLLBAR_AUTO);
    html_viewer->LoadPage(GUI::from_u8(full_file_path.string()));
    // when using hyperlink, open the browser.
    html_viewer->Bind(wxEVT_HTML_LINK_CLICKED, [this](wxHtmlLinkEvent &evt) { wxLaunchDefaultBrowser(evt.GetLinkInfo().GetHref()); });
...
void CalibrationAbstractDialog::on_dpi_changed(const wxRect &suggested_rect)
{
    ...
    Layout();
    this->SetSize(oldSize.x * this->scale_factor() / this->prev_scale_factor(),
                  oldSize.y * this->scale_factor() / this->prev_scale_factor());
    Refresh();
}
