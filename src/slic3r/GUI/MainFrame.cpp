...
static wxIcon main_frame_icon(GUI_App::EAppMode app_mode)
{
    ...
    if (len > 0 && len < MAX_PATH) {
        ...
        if (app_mode == GUI_App::EAppMode::GCodeViewer) {
            ...
            for (const std::wstring_view exe_name : { std::wstring_view(L"slicr-3d-v2.exe"), std::wstring_view(L"slicr-3d-v2-console.exe") })
                if (boost::iends_with(path, exe_name)) {
                    path.erase(path.end() - exe_name.size(), path.end());
                    path += L"slicr-3d-v2.exe";
                    break;
                }
        }
    }
...
//       _L("Remember to check for updates at https://github.com/prusa3d/PrusaSlicer/releases"));
    ...
    Bind(wxEVT_CLOSE_WINDOW, [this](wxCloseEvent& event) {
        ...
        if (m_plater != nullptr) {
            int saved_project = m_plater->save_project_if_dirty(_L("Closing SliCR-3D-V2. Current project is modified."));
            ...
            // check unsaved changes only if project wasn't saved
                     ...
                     (plater()->is_presets_dirty() && !wxGetApp().check_and_save_current_preset_changes(_L("SliCR-3D-V2 is closing"), _L("Closing SliCR-3D-V2 while some presets are modified.")))) {
                event.Veto();
                return;
            }
        }
