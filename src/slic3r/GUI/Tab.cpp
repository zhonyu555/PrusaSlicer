///|/ Copyright (c) Prusa Research 2017 - 2023 Oleksandra Iushchenko @YuSanka, Lukáš Matěna @lukasmatena, Lukáš Hejl @hejllukas, Vojtěch
///Bubník @bubnikv, Pavel Mikuš @Godrak, Tomáš Mészáros @tamasmeszaros, David Kocík @kocikdav, Enrico Turri @enricoturri1966, Vojtěch Král
///@vojtechkral
...
void Tab::update_wiping_button_visibility()
{
    ...
    bool single_extruder_multi_material =
        dynamic_cast<ConfigOptionBool *>((m_preset_bundle->printers.get_edited_preset().config).option("single_extruder_multi_material"))
            ->value;
...
void Tab::build_preset_description_line(ConfigOptionsGroup *optgroup)
{
    ...
    auto detach_preset_btn = [this](wxWindow *parent) {
        m_detach_preset_btn = new ScalableButton(parent, wxID_ANY, "lock_open_sys", _L("Detach from system preset"), wxDefaultSize,
...
void TabPrint::build()
{
    ...
    line            = {"", ""};
    line.full_width = 1;
    line.widget     = [this](wxWindow *parent) { return description_line_widget(parent, &m_post_process_explanation); };
    optgroup->append_line(line);
...
void TabFilament::update_filament_overrides_page()
{
    ...
    og_it = std::find_if(page->m_optgroups.begin(), page->m_optgroups.end(),
                         [](const ConfigOptionsGroupShp og) { return og->title == "Retraction when tool is disabled"; });
    if (og_it == page->m_optgroups.end())
        return;
    optgroup = *og_it;
...
{
    m_extruders_cb = new BitmapComboBox(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(12 * m_em_unit, -1), 0, nullptr,
                                        wxCB_READONLY);
    m_extruders_cb->Hide();

    m_extruders_cb->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent &) { set_active_extruder(m_extruders_cb->GetSelection()); });
    ...
    for (auto extruder_idx = m_extruders_count_old; extruder_idx < m_extruders_count; ++extruder_idx) {
        ...
        optgroup->m_on_change = [this, extruder_idx](const t_config_option_key &opt_key, boost::any value) {
            ...
            if (is_nozzle_diameter_changed) {
                if (extruder_idx == 0)
                    // Mark the print & filament enabled if they are compatible with the currently selected preset.
                    // If saving the preset changes compatibility with other presets, keep the now incompatible dependent presets selected,
                    // however with a "red flag" icon showing that they are no more compatible.
                    m_preset_bundle->update_compatible(PresetSelectCompatibleType::Never);
                else
        ...
        auto reset_to_filament_color = [this, extruder_idx](wxWindow *parent) {
            ScalableButton *btn = new ScalableButton(parent, wxID_ANY, "undo", _L("Reset to Filament Color"), wxDefaultSize,
                                                     wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
        ...
        line.widget                   = [this, extruder_idx](wxWindow *parent) {
            ScalableButton *btn = new ScalableButton(parent, wxID_ANY, "copy", _L("Apply below setting to other extruders"), wxDefaultSize,
                                                                       wxDefaultPosition, wxBU_LEFT | wxBU_EXACTFIT);
            auto            sizer = new wxBoxSizer(wxHORIZONTAL);
            sizer->Add(btn);
                    ...
                    if (tab->supports_printer_technology(printer_technology)) {
                        ...
                        if (!wxGetApp().tabs_as_menu()) {
                            std::string bmp_name = tab->type() == Slic3r::Preset::TYPE_FILAMENT     ? "spool" :
                                                   tab->type() == Slic3r::Preset::TYPE_SLA_MATERIAL ? "resin" :
                                                                                                      "cog";
        ...
        if (new_printer_technology == ptSLA && old_printer_technology == ptFFF &&
            ...
            for (PresetUpdate &pu : updates) {
                ...
                if (pu.tab_type == Preset::TYPE_FILAMENT && pu.new_preset_compatible) {
                    ...
                    const int active_extruder = dynamic_cast<const TabFilament *>(wxGetApp().get_tab(Preset::TYPE_FILAMENT))
                                                    ->get_active_extruder();
                    const int extruder_count_new = int(
                        dynamic_cast<const ConfigOptionFloats *>(new_printer_preset.config.option("nozzle_diameter"))->size());
...
wxSizer *Tab::compatible_widget_create(wxWindow *parent, PresetDependencies &deps)
{
    ...
    deps.btn = new ScalableButton(parent, wxID_ANY, "printer", format_wxstr(" %s %s", _L("Set"), dots), wxDefaultSize, wxDefaultPosition,
...
{
    ScalableButton *btn = new ScalableButton(parent, wxID_ANY, "printer", " " + _(L("Set")) + " " + dots, wxDefaultSize, wxDefaultPosition,
                                             wxBU_LEFT | wxBU_EXACTFIT);
    btn->SetFont(wxGetApp().normal_font());
    btn->SetSize(btn->GetBestSize());
...
{
    m_icon_descriptions
        .emplace_back(&m_bmp_value_lock, L("LOCKED LOCK"),
...
bool Tab::save_current_preset(const std::string &new_name, bool detach) { return m_presets->save_current_preset(new_name, detach); }
...
void TabSLAMaterial::build()
{
    ...
    create_tsmc(optgroup, "rest_time_after_lift");
    ...
    optgroup->m_on_change = [this](t_config_option_key opt_key, boost::any value) {
        ...
        if (opt_key == "tsmc_enable") {
            if (boost::any_cast<bool>(value) == true) {
                double new_height = new_conf.option("lift_distance")->getFloat() + new_conf.option("tsmc_lift_distance")->getFloat() -
