...
#else
    m_arrow.set_color(-1, { 0.75f, 0.0f, 0.0f, 1.0f });
#endif // ENABLE_COLOR_CLASSES
...
#else
    m_arrow.set_color(-1, { 0.0f, 0.75f, 0.0f, 1.0f });
#endif // ENABLE_COLOR_CLASSES
...
#else
    m_arrow.set_color(-1, { 0.0f, 0.0f, 0.75f, 1.0f });
#endif // ENABLE_COLOR_CLASSES
...
void Bed3D::render_texture(bool bottom, GLCanvas3D& canvas)
{
    ...
    if (m_texture.get_id() == 0 || m_texture.get_source() != m_texture_filename) {
        ...
        if (boost::algorithm::iends_with(m_texture_filename, ".svg")) {
            ...
            // generate a temporary lower resolution texture to show while no main texture levels have been compressed
            if (m_temp_texture.get_id() == 0 || m_temp_texture.get_source() != m_texture_filename) {
                if (!m_temp_texture.load_from_png_file(m_texture_filename, false, GLTexture::ECompressionType::None, false)) {
                    render_default(bottom, false);
                    return;
                }
                canvas.request_extra_frame();
            ...
            // starts generating the main texture, compression will run asynchronously
            if (!m_texture.load_from_png_file(m_texture_filename, true, GLTexture::ECompressionType::MultiThreaded, true)) {
                render_default(bottom, false);
                return;
            }
        }
