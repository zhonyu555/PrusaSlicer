...
static void start_new_slicer_or_gcodeviewer(const NewSlicerInstanceType instance_type, const std::vector<wxString> paths_to_open, bool single_instance, bool delete_after_load)
{
	...
	path += "\\";
	path += (instance_type == NewSlicerInstanceType::Slicer) ? "slicr-3d-v2.exe" : "slicr-3d-v2-gcodeviewer.exe";
...
#else // Linux or Unix
	{
		...
		if (args.empty()) {
			...
			my_path = (bin_path.parent_path() / ((instance_type == NewSlicerInstanceType::Slicer) ? "slicr-3d-v2" : "slicr-3d-v2-gcodeviewer")).string();
